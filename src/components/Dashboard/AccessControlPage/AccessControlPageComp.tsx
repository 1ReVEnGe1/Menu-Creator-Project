    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-2xl bg-amber-50 flex items-center justify-center text-lg shrink-0">
            🔒
          </div>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative w-full sm:w-80">
          <input
            ...
            className="w-full pl-4 pr-10 py-2.5 rounded-xl border border-slate-200 focus:outline-none transition-all text-sm text-slate-700 text-right"
          />
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${
                            role.isSystemRole ? "bg-amber-500" : "bg-emerald-500"
                          }`}
                        />
          ...
          filteredRoles.map((role) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-slate-800 font-mono text-sm">
                    {role.name}
                  </h4>
                ...
                <div className="flex flex-wrap gap-1">
                  {role.permissions.length > 0 ? (
                    role.permissions.map((perm) => (
                      <span
                        key={perm._id}
                        className="px-2 py-0.5 text-[10px] bg-slate-100 text-slate-600 rounded-md border border-slate-200/50"
                      >
                        {perm.name}
                      </span>
                    ))
